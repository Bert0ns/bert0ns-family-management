...
const templatePath = path.join(
  __dirname,
  '../.agents/skills/distributed-code-audit/references/audit-scorecard-template.html',
);
...
const findings = [
  ...
  // ==========================================
  {
    ...
    problem:
      'When importExpenseReport executes, it appends expenses to Zustand via set(), but never calls notifyStoreMutation. The syncEngine is never notified, so imported transactions remain local-only and are never uploaded to Supabase or shared with family members.',
...
notifyStoreMutation({ entity: 'member', operation: 'DELETE', entity_id: id, payload: { id } });`,
  },
  {
    ...
    problem:
      'importExpenseReport generates IDs formatted as exp_imp_${Date.now()}... whereas Supabase schema defines expenses.id as UUID NOT NULL. When the syncEngine attempts to sync imported expenses, PostgreSQL rejects them with code 22P02: invalid input syntax for type uuid.',
...
import_batch_id: batchId,`,
  },
  {
    ...
    problem:
      'When updateExpense is called with a modified amount, existing splits are retained unchanged unless explicitly supplied. A 100€ split (50€ each) updated to 200€ continues reporting 50€ shares, corrupting family debt calculations.',
...
if (updates.amount !== undefined && updates.amount !== e.amount && nextSplits && nextSplits.length > 0) {
  ...
  },
  {
    ...
    problem:
      'useAppStore is consumed across all screens without selectors or shallow equality checks. Any state update (such as typing a character in the ledger search bar) triggers a full-app re-render cascade across all active tab screens.',
...
export const useAppStoreShallow = <T>(selector: (state: AppState) => T): T => useAppStore(useShallow(selector));`,
  },
  {
    ...
    problem:
      'filters: FilterOptions is stored in the persistent AppState. Keystrokes in the search bar trigger AsyncStorage partialize serializations and root-level state invalidations on every frame.',
...
export const useFilterStore = create<FilterState>((set) => ({
  ...
  },
  {
    ...
    problem:
      'ImportBatch records store raw_payload: report (unparsed multi-megabyte JSON). The persist middleware runs JSON.stringify on the entire state on every update, blocking the JS thread and hitting Android AsyncStorage row limits.',
...
importBatches: state.importBatches.map(({ raw_payload, ...batch }) => batch),`,
  },
  {
    ...
    title:
      '[P1-08] Single Global Mutation Listener with No Multi-Subscriber or Unsubscribe Lifecycle',
    problem:
      'Mutation listener is a single module variable (let mutationListener). Registering another listener clobbers syncEngine silently, and no unsubscribe function is returned.',
...
export const registerStoreMutationListener = (listener: StoreMutationListener): (() => void) => {
  ...
  },
  {
    ...
    problem:
      'If only 1 category exists, fallbackCat evaluates to the very category being deleted. Also default categories (is_default: true) can be deleted, leaving the app without standard taxonomy.',
...
if (target?.is_default) return;`,
  },
  {
    ...
    problem:
      'reconcileRemoteExpenses performs nested findIndex lookups and uses unshift(), causing O(N^2) memory copies on large transaction histories.',
...
return { expenses: Array.from(map.values()).sort(sortByDateDesc) };`,
  },
  {
    ...
    problem:
      'Mutating state.categories = [...] directly in onRehydrateStorage callback bypasses Zustand internal subscription notifications and does not mark state dirty.',
  ...
  },
  {
    ...
    problem:
      'StoreMutationEvent.payload: any and OutboxMutation.payload: any bypass TypeScript compiler checking in store and sync engine.',
  ...
  // ==========================================
  {
    ...
    problem:
      'User input in merchant_name, notes, or payment_method beginning with =, +, -, @, \\t, \\r is wrapped in quotes but not prepended with an apostrophe. Opening the exported CSV in Excel or Google Sheets executes arbitrary formulas/DDE commands.',
  ...
  },
  {
    ...
    problem:
      'Balances with net of +0.01 or -0.01 are discarded by b.net < -0.01 and b.net > 0.01. Furthermore, if remaining debts cannot be paired with creditors, transfers is empty and isBalanced: transfers.length === 0 falsely reports true.',
...
const isBalanced = debtors.length === 0 && creditors.length === 0;`,
  },
  {
    ...
    problem:
      'checkDuplicate runs an array find() per candidate on every modal render. Merchant matching uses substring containment (exp.includes(candidate)), causing short names like "Bar" or "Apple" to match unrelated businesses.',
...
// Exact merchant matching or normalized tokens rather than substring containment`,
  },
  {
    ...
    problem:
      'z.number().positive() does not reject Infinity (Infinity > 0 is true). Passing Infinity crashes downstream analytics calculations. Missing string length bounds allow unbounded payloads to exhaust memory.',
...
merchant: z.string().trim().min(1).max(200),`,
  },
  {
    ...
    problem:
      'Without a UTF-8 Byte Order Mark (\\uFEFF), Excel on Windows and macOS interprets CSV files using legacy Windows-1252/ANSI, rendering € as â‚¬ and corrupting Italian characters like Caffè.',
  ...
  },
  {
    ...
    problem:
      'URL.createObjectURL(blob) is never released via URL.revokeObjectURL. Furthermore, fileExporter directly invokes Alert.alert with hardcoded English strings instead of returning a typed result to UI callers.',
...
// Return typed ExportResult: { success: boolean; error?: string }`,
  },
  {
    ...
    problem:
      'ReportValidator discards all validation issues except the first, forcing tedious one-by-one fixes. Paths like expenses.14.date confuse non-technical users.',
...
return issues.map((issue) => {
  ...
  },
  {
    ...
    problem:
      'calculateCategoryBreakdown loops strictly over categories. Any expense with a deleted or unmapped category ID is dropped, causing category totals to not sum up to totalSpend.',
  ...
  },
  {
    ...
    problem:
      'In months with 0 expenses, topCategory is undefined, but topSpender returns an arbitrary family member with €0.00 spend.',
  ...
  },
  {
    ...
    problem:
      'fileExporter.ts has 0% test coverage across web DOM and native expo-sharing branches.',
  ...
  // ==========================================
  {
    ...
    title:
      '[P3-01] Critical RLS Privilege Escalation: Global Family Enumeration & Membership Hijacking',
    problem:
      'families SELECT policy permits any authenticated user to view all families and invite codes. family_members INSERT policy allows any authenticated user to join any family without invite code verification.',
...
CREATE FUNCTION join_family_by_code(p_code TEXT) ...`,
  },
  {
    ...
    problem:
      'Supabase Realtime events for expenses table do not include split relations, mapping to splits: undefined. Reconciling remote expenses unconditionally overwrites local records, wiping out all split allocations.',
...
current[idx] = {
  ...
  },
  {
    ...
    problem:
      'flushOutbox lacks an isFlushing mutex. While an async flush is awaiting network I/O, concurrent enqueueMutation calls append items to AsyncStorage. When flushOutbox finishes, it overwrites storage with its stale snapshot, permanently losing newly enqueued mutations.',
...
// Introduce isFlushing lock. Upon flush completion, atomically filter out only successfully processed mutation IDs from the latest stored array.`,
  },
  {
    ...
    problem:
      'Delta sync queries updated_at > lastSync. Hard SQL DELETE removes the row completely, so offline clients never receive deletion notices and retain permanent zombie records.',
...
// In delta sync, if deleted_at IS NOT NULL, remove from local store.`,
  },
  {
    ...
    problem:
      'AuthModal unconditionally invokes migrateLocalDataToSupabase upon OTP verification. A user logging in on a secondary device with initial mock state overwrites established cloud data.',
...
if (memberships.length > 0) {
  ...
  },
  {
    ...
    problem:
      'RLS is enabled on budgets and import_batches without creating any policies. In PostgreSQL, this results in default-deny for all authenticated and anonymous roles.',
...
CREATE POLICY "Family members can manage budgets" ON public.budgets FOR ALL USING (public.is_member_of_family(family_id));`,
  },
  {
    ...
    title:
      '[P3-07] Family Pairing Fails to Insert Member Record and Contaminates Cross-Family Store State',
    problem:
      'Joining a family updates local store family ID but never creates a family_members row in Supabase (causing subsequent RLS blocks), and fails to wipe previous family records from the local store.',
  ...
  },
  {
    ...
    problem:
      'family.id captured in AuthModal closure is fam_1. After migration generates a new UUID, realtimeSync subscribes to fam_1 instead of the new UUID.',
...
realtimeSync.startRealtimeSync(currentFamilyId);`,
  },
  {
    ...
    title:
      '[P3-09] Non-Discriminant Retry Limit Silently Discards Outbox Mutations in Offline Mode',
    problem:
      'Mutations fail when offline. After 5 retries, mutations are permanently deleted from the queue without distinguishing between offline timeouts and fatal 4xx errors.',
  ...
  },
  {
    ...
    problem:
      'Supabase JWT access and refresh tokens are stored unencrypted in AsyncStorage on iOS and Android.',
...
// Implement ExpoSecureStoreAdapter for Supabase auth persistence on native.`,
  },
  {
    ...
    problem:
      'No B-tree indexes exist on family_members(family_id, user_id) or expenses(family_id, updated_at). Every RLS evaluation triggers sequential table scans.',
...
CREATE INDEX idx_expense_splits_exp_id ON public.expense_splits(expense_id);`,
  },
  {
    ...
    problem:
      'Channel errors never clear activeChannel, blocking re-subscription. Realtime is only bound in the settings tab, and app backgrounding/foregrounding is not tracked.',
  ...
  },
  {
    ...
    problem:
      'Expenses and splits are modified in separate HTTP calls. Network drops after deleting splits leave expenses orphaned with 0 splits.',
  ...
  },
  {
    ...
    problem:
      'navigator.clipboard is undefined on native iOS/Android, but modal shows success haptic and copied banner anyway.',
...
await Clipboard.setStringAsync(currentInviteCode);`,
  ...
  // ==========================================
  {
    ...
    problem:
      'When spend totals are 0 or empty, getY produces NaN or -Infinity. Passing NaN to React Native SVG Path elements causes native thread crashes or visual corruption.',
...
const getY = (val: number) => {
  ...
  },
  {
    ...
    problem:
      'When amount is empty, numericAmount is NaN. Because NaN <= 0 evaluates to false in JavaScript, calculateEqualSplits(NaN, ...) executes, persisting NaN split shares.',
  ...
  },
  {
    ...
    problem:
      "Avatar performs name.split(' ') without null guarding. When rendering an expense with an unknown or deleted member, it crashes with TypeError: Cannot read properties of undefined (reading 'split').",
  ...
  },
  {
    ...
    problem:
      'Tapping Delete in ExpenseDetailModal immediately deletes the expense without confirmation, leading to accidental transaction loss when users miss the close button.',
  ...
  },
  {
    ...
    problem:
      'If a category or member was deleted or modified remotely, lookups return undefined. Components access category.name without null checks, crashing with unhandled TypeError.',
...
const displayMember = member || DEFAULT_FALLBACK_MEMBER;`,
  },
  {
    ...
    problem:
      'new Date("YYYY-MM-DD") parses as UTC midnight, shifting back one day in negative timezones when converted to local time.',
...
const localDate = new Date(year, month - 1, day, 12, 0, 0);`,
  },
  {
    ...
    problem:
      'SplitCalculator.tsx does not import useI18n() and displays hardcoded English strings despite localization keys existing. Screens like index.tsx also contain hardcoded Italian phrases.',
...
// Replace hardcoded strings with t.addExpense.split* keys`,
  },
  {
    ...
    problem:
      'Color pickers (36px) and member chips in modals violate the WCAG AAA senior accessibility directive requiring 48-64px touch targets.',
  ...
  },
  {
    ...
    problem:
      'Full dataset filtering and sorting runs synchronously in the render body on every keystroke, while ExpenseItem is not wrapped in React.memo.',
  ...
  },
  {
    ...
    problem:
      'FormModal wraps children in a vertical ScrollView. LedgerScreen wraps filter content in an additional ScrollView, causing Android touch cancellation and console warnings.',
  ...
  },
  {
    ...
    problem:
      "Modal screen pushes tab route router.push('/(tabs)/import'), leaving modal context stacked above tab navigator.",
    fix: `router.dismiss();
router.navigate('/(tabs)/import');`,
  },
  {
    ...
    problem:
      'Context providers recreate fresh object values on every render, triggering full tree re-evaluations.',
  ...
  },
  {
    ...
    problem:
      'Heatmap cells and velocity charts lack accessibilityLabel and accessibilityRole for screen readers.',
  ...
  },
  {
    ...
    problem:
      'Truncates data to 6 items without grouping remaining categories into an "Other" slice, causing donut slices to sum to <100%.',
  ...
  },
  {
    ...
    problem:
      'FlatList in ledger.tsx defines renderItem inline and omits initialNumToRender and windowSize props.',
...
console.log(
  `Metrics: Total: ${totalFindings} | Critical: ${criticalCount} | Important: ${importantCount} | Suggestions: ${suggestionCount} | Nits: ${nitCount}`,
);
