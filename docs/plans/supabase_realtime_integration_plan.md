...
- Default role for invited family members: automatically make them full `MEMBER` (able to add, edit, and view expenses)? _(Recommended: Yes, frictionless for spouses/partners)_.
- Should we provide sample environment variable file (`.env.example`) with dummy values for easy setup? _(Recommended: Yes)_.
