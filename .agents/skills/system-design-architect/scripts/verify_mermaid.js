...
function lintMermaidBlock(block) {
  ...
  if (header.startsWith("sequenceDiagram")) {
    ...
    const participants = block.lines
      .filter(
        (l) =>
          l.trim().startsWith("participant ") || l.trim().startsWith("actor "),
      )
  ...
  if (
    header.startsWith("stateDiagram") ||
    header.startsWith("stateDiagram-v2")
  ) {
...
function scanAndLintDir(targetDir) {
  ...
  for (const file of files) {
    if (file.endsWith(".md")) {
      ...
      if (blocks.length > 0) {
        console.log(
          `📄 ${file} (${blocks.length} Mermaid block${blocks.length > 1 ? "s" : ""})`,
        );
        for (const b of blocks) {
          ...
          if (warnings.length > 0) {
            for (const warn of warnings) {
              totalWarnings++;
              console.warn(
                `  ⚠️  [L${warn.line}] [${warn.rule}] ${warn.message}`,
              );
            }
          }
        }
