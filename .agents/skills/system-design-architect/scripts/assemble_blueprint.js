...
function validateAndAssemble(targetDir, options = {}) {
  ...
  if (options.promote) {
    ...
    if (count > 0) {
      console.log(
        `🚀 Promoted ${count} file(s) from .staging/ to ${targetDir}`,
      );
    }
  }
  ...
  for (const mod of CANONICAL_MODULES) {
    ...
    if (!fs.existsSync(filePath)) {
      errors.push(
        `Missing canonical module file: ${mod.file} (Module ${mod.id})`,
      );
    } else {
  ...
  if (errors.length > 0) {
    ...
    console.log(
      "✨ All 10 architecture modules assembled and validated successfully!\n",
    );
    process.exit(0);
  }
}
...
function main() {
  ...
  const transcriptsArg = args
    .find((a) => a.startsWith("--from-transcripts="))
    ?.split("=")[1];
  ...
  if (unpackArg) {
    const payloadContent = fs.readFileSync(
      path.resolve(process.cwd(), unpackArg),
      "utf8",
    );
