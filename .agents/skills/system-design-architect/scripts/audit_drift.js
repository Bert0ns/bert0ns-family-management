...
function auditDrift(
  ...
  if (format === "json") {
    console.log(
      JSON.stringify(
        { baseRef, changedFileCount: diffFiles.length, driftResults },
        null,
        2,
      ),
    );
  ...
  console.log(
    "💡 Tip: Use Mode B targeted regeneration to update only affected modules.",
  );
  console.log("─".repeat(60) + "\n");
}
...
function main() {
  ...
  const baseRefArg =
    args.find((a) => a.startsWith("--base="))?.split("=")[1] || "HEAD~10";
